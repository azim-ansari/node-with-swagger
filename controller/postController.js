...
module.exports = {
  addPost: async (req, res) => {
    try {
      ...
      if (!title || !description) {
        return res
          .status(421)
          .json({ message: "Please Enter title and description" });
      }
  ...
  postDetails: async (req, res) => {
    try {
      ...
      console.log(postId);
      const data = await postSingleData(postId);
  ...
  postUpdate: async (req, res) => {
    try {
      const postId = req.params.id;
      const userId = req.user._id;
      const data = await updatedPost(postId, userId, req.body);
      if (data == null) {
        return res
          .status(404)
          .json({ message: "Post not created yet, please Add post" });
      }
  ...
  deletePost: async (req, res) => {
    try {
      ...
      const data = await postDelete(postId, userId);
